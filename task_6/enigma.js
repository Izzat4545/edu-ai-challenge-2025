...
class Enigma {
  ...
  stepRotors() {
    // Correct Enigma stepping logic with double-stepping anomaly:
    // - Rightmost rotor (rotor 2) always steps.
    // - Middle rotor (rotor 1) steps if it is at notch OR if the rightmost rotor is at notch (double-step).
    // - Leftmost rotor (rotor 0) steps if the middle rotor is at notch (double-step).
    const middleAtNotch = this.rotors[1].atNotch();
    const rightAtNotch = this.rotors[2].atNotch();
    // Double-stepping: if middle rotor is at notch, left rotor steps
    if (middleAtNotch) this.rotors[0].step();
    // If right rotor is at notch or middle rotor is at notch, middle rotor steps
  ...
  encryptChar(c) {
    ...
    // Only step rotors for alphabetic characters
    this.stepRotors();
